// app/api/listings/coming-soon/route.ts
...
export async function GET() {
  try {
    const properties = await prisma.property.findMany({
      where: {
        status: 'COMING_SOON', // <-- Filtrado por COMING_SOON
      },
      select: {
        id: true,
        slug: true,
    ...
    const listings = properties.map((p) => {
      ...
      return {
        ...
        interestRate: Number(p.interestRate) || null,
      };
    });
    ...
    return NextResponse.json(
      { error: 'Internal Server Error fetching coming soon listings.' },
      { status: 500 }
    );
  }
}