...
export default function MapSplitView({ properties, lang, t, searchType }: any) {
  ...
  // --- 1. NUEVA LÓGICA DE AUTO-DESELECCIÓN ---
  // Este efecto corre cada vez que cambia la lista de propiedades (por filtros o tabs)
  useEffect(() => {
    if (highlighted) {
      // Buscamos si la propiedad seleccionada existe en la nueva lista
      const stillExists = properties.find((p: any) => p.id === highlighted.id);
      
      // Si ya no existe (ej. cambiamos de Venta a Renta y esta casa era solo Venta)
      if (!stillExists) {
        setHighlighted(null); // Limpiamos la selección
        setIsMobileExpanded(false); // Cerramos el panel móvil si estaba abierto
      } else {
        // Opcional: Actualizamos los datos del highlighted con los nuevos datos frescos
        // (útil si cambió algún cálculo de precio)
    ...
    <div className="relative w-full h-full bg-[#0a0f1c] lg:flex lg:flex-row overflow-hidden">
          ...
          <div id="sidebar-list-container" className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-transparent">
             {orderedProperties.map((property: any) => (
                <DesktopCard 
                    ...
                    labelTexts={t.card}
                    specsTexts={t.specs} // 2. PASAMOS LOS TEXTOS DE SPECS
     ...
     <div className="absolute inset-0 w-full h-full lg:relative lg:flex-1 z-0 bg-[#0a0f1c] relative">
         <MapLoader 
            ...
            searchType={searchType} // <--- AGREGAR ESTO
         />
         
         <div className="hidden lg:flex flex-col items-start absolute top-6 left-6 z-[50] w-[90%] max-w-sm transition-all duration-300 ease-out">
      ...
      <div className="lg:hidden pointer-events-none absolute inset-0 z-30 flex flex-col justify-end">
          ...
          <div className="absolute top-0 left-0 w-full pointer-events-auto p-4 pt-20 flex flex-col items-start">
             <MapTabs currentType={searchType} texts={t.tabs} />
             <FloatingSearch placeholder={t.search.placeholder} onOpenFilters={() => setIsFilterOpen(true)} />
          </div>
          ...
          <div className={`pointer-events-auto bg-[#0a0f1c] rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.6)] border-t border-white/10 transition-all duration-500 ease-in-out flex flex-col ${isMobileExpanded ? 'h-[75vh]' : 'h-auto pb-6'} `}>
              ...
              <div id="mobile-list-container" className={`flex-1 overflow-y-auto px-4 pt-2 pb-safe space-y-4 scrollbar-hide ${!isMobileExpanded ? 'overflow-hidden' : ''} `}>
...
function DesktopCard({ property, isHighlighted, onClick, statusTexts, labelTexts, specsTexts, isMobile = false, searchType = 'buy' }: any) {
    ...
    return (
        <div 
            onClick={onClick} 
            className={`
                cursor-pointer rounded-2xl overflow-hidden border transition-all duration-300 group flex flex-col
                ${isHighlighted 
        ...
        >
            ...
            <div className="p-4 flex flex-col gap-3">
                ...
                {/* Precios */}
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 border-b border-gray-800 pb-3">
                    <h3 className="text-xl font-bold text-gray-300 tracking-tight flex gap-1">
                        {isRentMode && <span className="text-xs self-center uppercase text-gray-500 mr-1">Dep:</span>}
                ...
                <div>
                    ...
                    {/* 3. USO DE TEXTOS BILINGÜES */}
                    <div className="flex items-center gap-3 text-gray-400 text-xs font-medium">
                        <span className="bg-gray-800/50 px-2 py-1 rounded"><strong className="text-white">{property.beds}</strong> {specsTexts?.beds || 'Beds'}</span>
                        <span className="bg-gray-800/50 px-2 py-1 rounded"><strong className="text-white">{property.baths}</strong> {specsTexts?.baths || 'Baths'}</span>
                        <span className="bg-gray-800/50 px-2 py-1 rounded"><strong className="text-white">{property.sqft}</strong> {specsTexts?.sqft || 'Sqft'}</span>
                    </div>
                </div>
            </div>
        </div>
