...
export default function WhatsAppButton({ 
  ...
  const positionClass = position === 'left' 
    ...
    // Reduje right-4 a right-2 y md:right-6 a md:right-3
    : 'right-2 md:right-3'; 
    
  const translateClass = position === 'left' ? '-translate-x-4' : 'translate-x-4';

  return (
    // También ajusté bottom-16 para que no quede muy pegado al borde inferior si se ve en pantallas pequeñas
    <div className={`fixed bottom-16 z-[55] flex items-center gap-2 group ${positionClass} transition-all duration-300`}>
      ...
      <Link 
        ...
        // Reduje w-14 a w-10 y h-14 a h-10
        className="relative flex items-center justify-center w-10 h-10 bg-[#25D366] rounded-full shadow-[0_4px_15px_rgba(37,211,102,0.4)] hover:scale-110 transition-transform"
      >
        <span className="absolute inset-0 rounded-full bg-[#25D366] animate-ping opacity-75"></span>
        {/* Reduje w-8 a w-6 y h-8 a h-6 para el icono */}
        <svg className="w-6 h-6 text-white relative z-10" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
